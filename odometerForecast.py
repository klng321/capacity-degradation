...
class odometerForecast:
    def __init__(self, dataReadPathPre, dataSavaPathPre, batchSize, nEpochs, outputLen):
        ...
        for vinMask in os.listdir(dataReadPathPre + r'/odoTransTrain'):  # 车辆数据集合并
            print(vinMask)
            df = pd.read_csv(filepath_or_buffer=dataReadPathPre + r'/odoTransTrain/%s' % vinMask, header=None)
            dataSet = df.to_numpy()
        ...
        for vinMask in os.listdir(dataReadPathPre + r'/odoTransTest'):
            df = pd.read_csv(filepath_or_buffer =dataReadPathPre + r'/odoTransTest/%s' % vinMask, header=None)
            testSet = df.to_numpy()
    ...
    def fitLSTM(self):
        ...
        # self.model.save(r'./result/3th/odoMeter/soh_model')
    ...
    def forecast(self):
        ...
        for j in range(len(self.vinTestCounts)):
            ...
            for i in range(self.vinTestCounts[j]):
                ...
                # testPredictSmooth.append(math.sqrt(mse(testExpect, testPredictSmooth)))  # 最后一位存储误差
            ...
            pd.DataFrame(dic).to_csv(self.dataSavePathPre+r'/%s.csv'%self.vinMasks[j], index=False)
            # pd.DataFrame(list(dic_mse.items()), columns=['vin', 'rmse']).to_csv(r'./result/odoMeter/误差.csv',index=False)
        # region 未来SOH预测
            # 未来soh预测
...
class xgbOdoForecast:
    def __init__(self, dataReadPathPre, dataSavePathPre,outputLen):
        ...
        self.gs = GridSearchCV(estimator=self.model, param_grid=parameters, cv=5, refit=True, scoring='neg_mean_squared_error')
...
# omForecast = odometerForecast(r'./data/discharge/2nd',r'./result/2nd/odoMeter',14,142,30) #输入1次，输出30段，未使用降噪
# omForecast = odometerForecast(r'./data/discharge/4th',r'./result/4th/odoMeter',14,142,30) #输入1次，输出30段，使用lasso降噪15%
# omForecast = odometerForecast(r'./data/discharge/4th_leastSquareReduce',r'./result/4th_leastSquareReduce/odoMeter',14,142,30) #输入1次，输出30段，使用最小二乘降噪15%
# omForecast = odometerForecast(r'./data/discharge/4th_maxminReduce',r'./result/4th_maxminReduce/odoMeter',14,142,30) #输入1次，输出30段，使用去除极值降噪15%

#region  论文补做实验 SG-filter reduction

# omForecast = odometerForecast(r'./data/discharge/4th_SGReduce',r'./result/4th_SGReduce/odoMeter',16,1,30) #输入1次，输出30段，使用最小二乘降噪15%
# omForecast = odometerForecast(r'./data/discharge/4th_SGReduce_0.25',r'./result/4th_SGReduce_0.25/odoMeter',16,1,30) #输入1次，输出30段，使用最小二乘降噪15%
omForecast = odometerForecast(r'./data/discharge/4th_SGReduce_0.35',r'./result/4th_SGReduce_0.35/odoMeter',16,1,30) #输入1次，输出30段，使用最小二乘降噪15%
omForecast.fitLSTM()
omForecast.forecast()

#endregion

# omForecast = odometerForecast(r'./data/discharge/5th',r'./result/5th/odoMeter',14,142,20)
# omForecast = odometerForecast(r'./data/discharge/5th_lassoReduce',r'./result/5th_lassoReduce/odoMeter',14,142,20)
# omForecast = odometerForecast(r'./data/discharge/5th_lassoReduce',r'./result/5th_lassoReduce/odoMeter',14,142,20)
# omForecast = odometerForecast(r'./data/discharge/5th_leastSquareReduce',r'./result/5th_leastSquareReduce/odoMeter',14,142,20)
# omForecast = odometerForecast(r'./data/discharge/5th_maxminReduce',r'./result/5th_maxminReduce/odoMeter',14,142,20)

# omForecast = odometerForecast(r'./data/discharge/6th',r'./result/6th/odoMeter',14,142,25)
# omForecast = odometerForecast(r'./data/discharge/6th_lassoReduce',r'./result/6th_lassoReduce/odoMeter',14,142,25)
# omForecast = odometerForecast(r'./data/discharge/6th_leastSquareReduce',r'./result/6th_leastSquareReduce/odoMeter',14,142,25)
# omForecast = odometerForecast(r'./data/discharge/6th_maxminReduce',r'./result/6th_maxminReduce/odoMeter',14,142,25)

# omForecast = odometerForecast(r'./data/discharge/7th',r'./result/7th/odoMeter',14,142,16)
# omForecast = odometerForecast(r'./data/discharge/7th_lassoReduce',r'./result/7th_lassoReduce/odoMeter',14,142,16)
# omForecast = odometerForecast(r'./data/discharge/7th_leastSquareReduce',r'./result/7th_leastSquareReduce/odoMeter',14,142,16)
# omForecast = odometerForecast(r'./data/discharge/7th_maxminReduce',r'./result/7th_maxminReduce/odoMeter',14,142,16)

#region 批量运行
# counts = ['4th']
# predLens = [30]
# noiseTypes = ['leastSquareReduce']
# # noiseTypes = ['maxminReduce']
# predLengths = dict(zip(counts,predLens))
#
# for count in counts:
#     for noiseType in noiseTypes:
#         for noiseData in ['0.25','0.35']:
#             path = f'./data/discharge/{count}_{noiseType}_{noiseData}'
#             if count == '6th' and noiseType == 'lassoReduce': #( noiseType != 'maxminReduce' or noiseRatio != '0.35' ) :
#                 print(f'Train is skipped for: {path}')
#                 continue
#             print(f'Train is start for: {path}')
#             omForecast = odometerForecast(path,f'./result/{count}_{noiseType}_{noiseData}/odoMeter',14,142,predLengths[count])
#             omForecast.fitLSTM()
#             omForecast.forecast()
#             del omForecast
#             print(f'Train is over for: {path}')

# omForecast.loadModel()
# endregion

# region xgbBoost模型预测
# # xgbForecast = xgbOdoForecast(r'./data/discharge/2nd',r'./xgbResult/2nd/odoMeter',30)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/3th',r'./xgbResult/3th/odoMeter',30)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/4th',r'./xgbResult/4th/odoMeter',30)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/4th_leastSquareReduce',r'./xgbResult/4th_leastSquareReduce/odoMeter',30)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/4th_maxminReduce',r'./xgbResult/4th_maxminReduce/odoMeter',30)

# # xgbForecast = xgbOdoForecast(r'./data/discharge/5th',r'./xgbResult/5th/odoMeter',20)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/5th_lassoReduce',r'./xgbResult/5th_lassoReduce/odoMeter',20)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/5th_leastSquareReduce',r'./xgbResult/5th_leastSquareReduce/odoMeter',20)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/5th_maxminReduce',r'./xgbResult/5th_maxminReduce/odoMeter',20)

# # xgbForecast = xgbOdoForecast(r'./data/discharge/6th',r'./xgbResult/6th/odoMeter',25)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/6th_lassoReduce',r'./xgbResult/6th_lassoReduce/odoMeter',25)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/6th_leastSquareReduce',r'./xgbResult/6th_leastSquareReduce/odoMeter',25)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/6th_maxminReduce',r'./xgbResult/6th_maxminReduce/odoMeter',25)

# # xgbForecast = xgbOdoForecast(r'./data/discharge/7th',r'./xgbResult/7th/odoMeter',16)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/7th_lassoReduce',r'./xgbResult/7th_lassoReduce/odoMeter',16)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/7th_leastSquareReduce',r'./xgbResult/7th_leastSquareReduce/odoMeter',16)
# # xgbForecast = xgbOdoForecast(r'./data/discharge/7th_maxminReduce',r'./xgbResult/7th_maxminReduce/odoMeter',16)



# # xgbForecast1 = xgbOdoForecast(r'./data/discharge/7th_lassoReduce_0.25',r'./xgbResult/7th_lassoReduce_0.25/odoMeter',16)
# # xgbForecast2 = xgbOdoForecast(r'./data/discharge/7th_leastSquareReduce_0.25',r'./xgbResult/7th_leastSquareReduce_0.25/odoMeter',16)
# # xgbForecast3 = xgbOdoForecast(r'./data/discharge/7th_maxminReduce_0.25',r'./xgbResult/7th_maxminReduce_0.25/odoMeter',16)
# # xgbForecast4 = xgbOdoForecast(r'./data/discharge/7th_lassoReduce_0.35',r'./xgbResult/7th_lassoReduce_0.35/odoMeter',16)
# # xgbForecast5 = xgbOdoForecast(r'./data/discharge/7th_leastSquareReduce_0.35',r'./xgbResult/7th_leastSquareReduce_0.35/odoMeter',16)
# # xgbForecast6 = xgbOdoForecast(r'./data/discharge/7th_maxminReduce_0.35',r'./xgbResult/7th_maxminReduce_0.35/odoMeter',16)

# xgbForecast1 = xgbOdoForecast(r'./data/discharge/6th_lassoReduce_0.25',r'./xgbResult/6th_lassoReduce_0.25/odoMeter',25)
# xgbForecast2 = xgbOdoForecast(r'./data/discharge/6th_leastSquareReduce_0.25',r'./xgbResult/6th_leastSquareReduce_0.25/odoMeter',25)
# xgbForecast3 = xgbOdoForecast(r'./data/discharge/6th_maxminReduce_0.25',r'./xgbResult/6th_maxminReduce_0.25/odoMeter',25)
# xgbForecast4 = xgbOdoForecast(r'./data/discharge/6th_lassoReduce_0.35',r'./xgbResult/6th_lassoReduce_0.35/odoMeter',25)
# xgbForecast5 = xgbOdoForecast(r'./data/discharge/6th_leastSquareReduce_0.35',r'./xgbResult/6th_leastSquareReduce_0.35/odoMeter',25)
# xgbForecast6 = xgbOdoForecast(r'./data/discharge/6th_maxminReduce_0.35',r'./xgbResult/6th_maxminReduce_0.35/odoMeter',25)
...
#     print(datetime.datetime.now())
#     xgbForecast = xgbOdoForecast(f'./data/discharge/{Pic[i]}',
#                                        f'./xgbResult/{Pic[i]}/odoMeter', predlen[i])
